...
namespace LetsEncrypt.Azure.Core.V2
{
    public class AcmeClient
    {
        ...
        public async Task<CertificateInstallModel> RequestDnsChallengeCertificate(IAcmeDnsRequest acmeConfig)
        {
            ...
            var order = await acmeContext.NewOrder(new[] { "*." + idn.GetAscii(acmeConfig.Host.Substring(2)), idn.GetAscii(acmeConfig.Host.Substring(2)) });
            var a = await order.Authorizations();

            var dnsTxts = new List<string>();

            foreach (var authz in a)
            {
